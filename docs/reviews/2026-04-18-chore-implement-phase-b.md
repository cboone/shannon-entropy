...
- **`entropyBits_unique_eq` naming** (resolved in commit `bc31a19`): the original `_const` suffix could parse as "constant entropy", which was misleading. The lemma was renamed to `entropyBits_unique_eq` to match the named-constant convention.

**Suggestions**:

- `ShannonTest/Entropy/Bits.lean`: the second uniqueness example is a fine mirror of the existential form but arguably redundant with the `Final.lean` sibling; consider trimming to just the named-constant `entropyBits_unique_eq` example here and leaving the existential to `Final.lean`, to keep the mirror test surface tight.
